document.addEventListener('DOMContentLoaded', async () => {
    ...
    const getScheduleCounts = (q) => {
        const counts = new Map();
        q.forEach(question => {
            // Only count non-completed, future (or today's) revisions
            const todayStr = getTodayStr();
            question.revisionDates.forEach(date => {
                if (date >= todayStr && !question.completedDates.includes(date)) {
    ...
    // This function no longer blocks the app's startup.
    const syncTime = async () => {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                // Re-render the UI after a successful sync to reflect the correct date.
                updateUI(); 
                return; 
            } catch (error) {
    ...
    const handleLogin = async (e) => {
        ...
        if (error) {
            ...
            // The onAuthStateChange listener will handle fetching data and showing the app
        }
    };
    ...
    const handleLogout = async () => {
        // The onAuthStateChange listener will handle showing the login screen
    ...
    // --- FIX START: REPLACED init() FUNCTION ---
    ...
    const init = async () => {
        ...
        // This variable tracks the currently loaded user
        // to prevent re-fetching data on token refreshes.
        let lastUserId = null;

        supabaseClient.auth.onAuthStateChange(async (event, session) => {
            const user = session?.user || null;
            
            console.log('Auth event:', event, 'Session User:', user?.id);

            if (user && user.id !== lastUserId) {
                // --- Case 1: A user is now signed in (and was not before) ---
                // This handles both initial page load with a valid session AND a new login.
                console.log(`Auth change: User ${user.id} detected. Fetching data.`);
                lastUserId = user.id;
                currentUser = user;
                await fetchInitialData(); // Fetch data ONCE
                showApp();
            } else if (!user && lastUserId !== null) {
                // --- Case 2: A user was signed in, but now is not ---
                // This handles a user-initiated logout.
                console.log('Auth change: User signed out.');
                lastUserId = null;
                showLogin();
            } else if (user && user.id === lastUserId) {
                // --- Case 3: Event fired, but the user is the same ---
                // This is likely a TOKEN_REFRESHED event (e.g., tab re-focus).
                // We DON'T re-fetch all data.
                console.log(`Auth event: ${event}. User ${user.id} is already loaded. No-op.`);
                // Just ensure the app screen is visible in case it was hidden
                showApp();
            } else if (!user && lastUserId === null) {
                // --- Case 4: Event fired, and there is no user (and wasn't before) ---
                // This is the initial page load without a session.
                console.log('Auth event: No user. Showing login screen.');
                showLogin();
            }
        });
    ...
    const updateUI = () => {
        // Guard against running UI updates if data isn't loaded
        if (!currentUser && !loginScreen.classList.contains('hidden')) {
            return; // Don't update app UI if we're on the login screen
        }
    ...
    const updateStreak = async (questionsToSave = getQuestions()) => {
        ...
        if (currentStats.lastCompletedDate === todayStr) return; // Already updated today
        ...
        await checkRewards(currentStats); // Check for rewards before saving
    ...
    const handleFormSubmit = async (e) => {
        ...
        if (intervalsInput && intervals.length === 0) {
            showAlert("Invalid revision intervals. Please use comma-separated numbers (e.g., 3, 7, 15).");
            return;
        }
        ...
        // --- MODIFIED LOGIC: Load balancing ---
        // The first interval is now "strict" and will be scheduled
        // regardless of the daily limit. Subsequent intervals will be balanced.
        for (const days of intervals) {
            let revisionDate = new Date(baseStartDate);
            revisionDate.setDate(revisionDate.getDate() + days);
            
            let revisionDateStr = dateToYYYYMMDD(revisionDate);

            // Check if this is the FIRST interval (e.g., the 3-day one)
            if (intervals.indexOf(days) === 0) {
                // This is the first interval. Schedule it strictly on this day,
                // regardless of the MAX_QUESTIONS_PER_DAY limit.
                // The date is already set correctly, so we do nothing.
            } else {
                // This is a subsequent interval (e.g., 7, 15, 30 days).
                // Apply load balancing: keep checking next days until we find a slot.
                while ((scheduleCounts.get(revisionDateStr) || 0) >= MAX_QUESTIONS_PER_DAY) {
                    // This day is full. Increment the date and check again.
                    revisionDate.setDate(revisionDate.getDate() + 1);
                    revisionDateStr = dateToYYYYMMDD(revisionDate);
                }
            }

            // We found an available slot (or forced it for the first interval)!
            revisionDates.push(revisionDateStr);

            // IMPORTANT: Update the local counts map for the *next* interval calculation
            // This prevents multiple intervals from this same question from landing on the same day
            // or affecting the balancing of the *next* interval.
            scheduleCounts.set(revisionDateStr, (scheduleCounts.get(revisionDateStr) || 0) + 1);
        }
        // --- END MODIFIED LOGIC ---
        ...
        const newQuestion = {
            ...
            revisionDates: revisionDates, // Use the new balanced dates
            completedDates: []
        };
        ...
        form.reset();
        questionDifficulty.value = 'Medium'; // Reset default
    ...
    const handleEditFormSubmit = async (e) => {
        ...
        let updatedQuestions = JSON.parse(JSON.stringify(getQuestions())); // Deep copy
    ...
    const deleteQuestion = (id) => {
        openConfirmModal('Delete Question?', 'This will permanently delete the question and all its revision data.', 'Delete', async () => {
    ...
    const toggleRevisionDone = async (id, date) => {
        let currentQuestions = JSON.parse(JSON.stringify(getQuestions())); // Deep copy
        ...
        if (dateIndex === -1) {
            ...
            // Check if this was the last item for today to update the streak
            if (date === getTodayStr()) {
                ...
                if (allTodaysItems.length > 0 && allTodaysCompleted) { 
                     // This was the last item, update streak and save
                     await updateStreak(currentQuestions);
                     return; // updateStreak already saves and updates UI
                }
            }
        } else {
        ...
        // Save without updating streak (since it wasn't the last item or was an un-check)
    ...
    const renderRevisions = (q) => {
        ...
        sortedDates.forEach(date => {
            ...
            else if (date < todayStr) dateHeaderClass = "text-lg font-semibold text-gray-500 dark:text-gray-400"; // Overdue
    ...
    const renderAllQuestionsList = (q) => {
        ...
        const ul = document.createElement('ul');
        ul.className = 'space-y-3';
        filteredQuestions.sort((a, b) => b.id - a.id); // Show newest first
